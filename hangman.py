    ...
    def check_letter(self, letter) -> None:
        ...
        if self.letter in self.word:
         for i in (idx for idx , i in enumerate(self.word) if i == self.letter):
                ...
                # catering for the case where the letter appears twice in the word hasnt been fixed yet
                self.word_guessed[i] = self.letter
                self.num_letters -= 1
        else:
            self.num_lives -= 1
...
# ensure all works for scoring and tick tasks con portal, need to send link to aicore instructor for review (Bola) by monday - done
